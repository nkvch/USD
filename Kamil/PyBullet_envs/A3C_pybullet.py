...
import gym
# import gymnasium as gym # Gymnasium not supported
import numpy as np
...
args = Args()
args.env_id ="HalfCheetahBulletEnv-v0" # "Walker2DBulletEnv-v0" 
...
def make_env(env_id, capture_video=False, run_dir="."):
    def thunk():
        if capture_video:
            env = gym.make(env_id, render=capture_video, max_episode_steps=1000)
            env.reset()
...
def worker(worker_id, global_policy, global_optimizer, args, global_step, run_dir):
    ...
    env = gym.make(args.env_id, render=False, max_episode_steps=1000)
    env.reset()
    env.render(mode="human")
